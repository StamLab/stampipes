...
script_options = {"processing": "processing.json"}
...
def get_max_mismatch_level(lane_set, index_count):
    mismatch_choices = list(
        itertools.product(POSSIBLE_MISMATCH_LEVELS, repeat=index_count)
    )
    lanes = lane_set.values()  # We don't actually care about lane labels (the key)
    for choice in mismatch_choices:
        no_collisions = all(
            [is_mismatch_level_okay(barcodes, choice) for barcodes in lanes]
        )
        if no_collisions:
            return choice
    return None
...
def main(args=sys.argv):
    ...
    lanes = create_lane_set(
        data["libraries"], mask, poptions.ignore_failed_lanes, poptions.allow_collisions
    )
