...
def parser_setup():
    ...
    parser.add_argument(
        "-b", "--basedir", dest="basedir", help="The base flowcell directory"
    )
    parser.add_argument(
        "-j", "--json", action="store_true", help="Write output in JSON"
    )
    parser.add_argument(
        "-t",
        "--threshold",
...
def get_input_files_for_lane(data, lane, basedir):
    ...
    globs.append(
        "Undetermined_indices/Sample_lane%d/lane%d_Undetermined_L%03d_R1_???.barcodes.txt"
        % (lane, lane, lane)
    )
    for g in globs:
        files += glob.glob(g)
...
def print_stats_txt(stats, threshold):
    for lane in stats.keys():
        ...
        for barcode in sorted(
            lane_stats.keys(),
            key=lambda x: lane_stats[x]["cluster_count"],
            reverse=True,
        ):
...
def print_stats_json(stats, threshold):
    for lane in stats.keys():
        stats[lane] = {
            k: v
            for k, v in stats[lane].iteritems()
...
def main(args=sys.argv):
    ...
    compiled_stats = {}
    for lane in lanes:
        barcode_files = get_input_files_for_lane(
            processing_data, lane, poptions.basedir
        )
        actual_barcodes = sum_barcodes(barcode_files)

        compiled_stats[lane] = merge_actual_and_expected(
            expected[lane], actual_barcodes
        )
