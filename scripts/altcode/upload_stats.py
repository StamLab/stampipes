...
JSON_REPORT_CLASS_SLUG = "altcode-flowcell-report-starsolo"
...
class UploadLIMS:
    ...
    def upload_altcode_flowcell(self, sample_config, processing_dict, outdir):
        ...
        for row in sample_config:
            ...
            lane_ids = [
                lane["id"]
                for lane in processing_dict["libraries"]
                if lane["barcode1"]["reverse_sequence"] == idx
                and int(lane["lane"]) == lane
            ]
...
def find_stat_files_in_dir(flowcell_directory):
    ...
    """
    path = "%s/Project_*/LibraryPool_*/align*" % flowcell_directory
    LOG.debug("Searching path %s", path)
    align_dirs = glob.glob(path)
    LOG.debug("Align dirs: %s", align_dirs)
    newest_stat_files = []
    ...
    for align_dir in align_dirs:
        ...
        out_dirs = glob.glob("%s/output*" % align_dir)
        LOG.debug("Considering: %s", out_dirs)
        if len(out_dirs) == 0:
            LOG.error("No output directories in %s", align_dir)
            continue
        out_dir = max(out_dirs, key=sortkey)
        LOG.debug("Selected:    %s", out_dir)
        file = os.path.join(out_dir, "stats.json")
        newest_stat_files.append(file)
    return newest_stat_files
...
def main():
    ...
    LOG.debug("Stats files: %s", stats_files)
    all_stats = []
    for file_name in stats_files:
        try:
            ...
            LOG.error("Could not read file %s: %s", file_name, e)
    LOG.debug("All stats: %s", all_stats)
