...
def parser_setup():
    ...
    parser.add_argument(
        "--project", dest="project", help="Run for alignments in this project."
    )
    ...
    parser.add_argument(
        "--auto_aggregate",
        dest="auto_aggregate",
        help="Script created will also run auto-aggregations after alignments finished.",
        action="store_true",
    )
    parser.add_argument(
        "--align_base_dir",
...
class ProcessSetUp(object):
    ...
    def setup_alignments(self, align_ids):
        for align_id, error in self.pool.map(self.setup_alignment, align_ids):
            if error:
                logging.warning("ALN%d result received, error: %s", align_id, error)
            else:
                logging.debug("ALN%d result received, OK", align_id)
    ...
    def create_script(self, processing_info, align_id):
        ...
        if (
            processing_info["libraries"]
            and processing_info["libraries"][0]
            and processing_info["libraries"][0].get("library_kit_method")
        ):
