...
class ClusterMonitor(object):
    ...
    def update_processing_status(self, align_id, processing=True):
        ...
        update_result = requests.patch(
            patch_url, headers=self.headers, data={"currently_processing": processing}
        )
    ...
    def lims_currently_processing(self):
        fetch_results = self.api_list_result(
            "flowcell_lane_alignment/?currently_processing=True"
        )
    ...
    def update_host_info(self):
        ...
        update = requests.patch(
            key_value["url"], data={"value": host_usage}, headers=self.headers
        )
